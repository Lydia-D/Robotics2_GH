...
% velObs = load('MvelocityObs.txt');
% gpsObs = load('MpositionObs.txt');
% compObs = load('McompassObs.txt');
% lasObs = load('MlaserObs.txt');
% lasFeats = load('MlaserFeatures.txt');


lasPure = lasObs(:, 3:2:length(lasObs(1,:))).* ...
    (lasObs(:, 3:2:length(lasObs(1,:)))<8);
lasMarkers = lasObs(:, 4:2:length(lasObs(1,:)));
%globalSize = [-15:15; -15:15];
globalWidth = 30;
gridBox = 0.5;
...
%% Grid positioning
globalPos = [round(X_fus(end)*gridBox/globalWidth+globalWidth/(2*gridBox),0), round(Y_fus(end)*gridBox/globalWidth+globalWidth/(2*gridBox),0)];
...
while 1
    ...
    elseif sensor == 3
%         [x_las, y_las, phi_las, grid_ij_local] = LasPose2(las_ind, X_Obs, Y_Obs, ...
