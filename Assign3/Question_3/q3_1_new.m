...
while 1
    ...
    elseif sensor == 3
        ...
        %wall
        wallData = lasWall(las_ind, X_fus(end), Y_fus(end), Phi_fus(end), lasPure, wallMarkers);
        %Update global grid
        gg = gg + localgrid(wallData, boxsize, rd);
...
gg = gg/max(max(gg));
imshow(gg);
saveas(gcf, strcat('Greyscale_Boxsize_', num2str(0.25),'.png'));
...
imshow(gg);
saveas(gcf, strcat('Threshold_', num2str(threshold),'_Boxsize_',num2str(boxsize),'.png'));
